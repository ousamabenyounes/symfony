...
class DebugCommandTest extends TestCase
{
    ...
    public function testExecuteWithStatefulScheduleUsesStoredCheckpoint()
    {
        ...
        $this->assertStringContainsString('stateful', $display, 'Output should mention the stateful checkpoint base date.');
        $this->assertStringContainsString('Mon, 15 Jan 2024 11:00:00 +0000', $display, 'Next run for an "every 1 hour" trigger seeded with checkpoint 2024-01-15 10:00:00 UTC must be 11:00:00 UTC, not "now + 1h".');
    }

    public function testExecuteWithStatefulScheduleWithoutCheckpointFallsBackToNow()
    {
        // Stateful schedule but the cache has NO stored checkpoint yet
        // (worker hasn't run, or entry was evicted). Helper must return null,
        // command must NOT mention "stateful" and fall back to "now" base date.
        $cache = new ArrayAdapter();
        ...
        $display = $tester->getDisplay(true);
        $this->assertStringNotContainsString('stateful', $display, 'No checkpoint stored — command must not announce a stateful base date.');
        $this->assertMatchesRegularExpression('/every 1 hour\s+stdClass\s+\w{3}, \d{1,2} \w{3} \d{4} \d{2}:\d{2}:\d{2} (\+|-)\d{4}/', $display, 'Next run must still be rendered using "now" as the base date.');
    }
}
